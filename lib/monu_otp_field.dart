...
class MonuOtpTextField extends StatefulWidget {
  ...
  @override
  _MonuOtpTextFieldState createState() => _MonuOtpTextFieldState();
}

class _MonuOtpTextFieldState extends State<MonuOtpTextField> {
  ...
  @override
  void didUpdateWidget(covariant MonuOtpTextField oldWidget) {
        ...
        onKey: (RawKeyEvent event) {
          if (event.runtimeType == RawKeyDownEvent) {
            if (event.logicalKey == LogicalKeyboardKey.backspace) {
              if (_textControllers[index]?.text.length == 0) {
                if (index != 0) {
                  FocusScope.of(context)
                      .requestFocus(_focusNodes[index - 1] ?? FocusNode());
                }
              } else {
            ...
            if (event.logicalKey == LogicalKeyboardKey.digit0 ||
              ...
              if (_textControllers[index]!.value.text.isEmpty) {
                /*_textControllers[index]?.text = event.logicalKey.keyLabel;
                ...
                if (index != widget.numberOfFields - 1) {
                  _textControllers[index + 1]?.text = event.logicalKey.keyLabel;

                  FocusScope.of(context)
                      .requestFocus(_focusNodes[index + 1] ?? FocusNode());
                } else {
                  if (widget.onSubmit != null) {
                    widget.onSubmit!(_textControllers[index]!.value.text);
                  }
                }
              }
            }
